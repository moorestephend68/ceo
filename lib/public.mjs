...
export async function joinPublic(db, { name, companyId, now }) {
  const at = now || new Date().toISOString();
  let game = await db.openPublicGame();
  let created = false;
...
export async function scoreGame(db, game) {
  if (!game || !game.isPublic || game.status !== 'over') return { skipped: 'not a finished public game' };
  ...
  try {
    await db.saveResults(game.code, placed.map((r) => ({
      ...
      rating_delta: (byKey[r.companyId || r.name] || {}).delta || 0,
      was_bot: r.isBot,
    })));
  } catch (e) {
