...
/* Measured, not guessed (test/seats.mjs, 24 seasons per cell):
     8 rounds  — median company $276-288k from $250k, no bankruptcies, at 3/4/5/6 seats
     12 rounds — median $157-196k, 0.13-0.38 bankruptcies a game
   Seat count barely matters: every seat brings its own customers as well as its
   own fixed costs. Length matters a lot, because the product decline that starts
   at round 8 has no answer in a one-product game — in the solo levels you launch
   something new, and live sessions cannot do that yet. So the range stops at 14. */
export const LIMITS = {
  seats: { min: 3, max: 6, default: 4 },
  rounds: { min: 8, max: 14, default: 10 },
};
...
export function createGame(opts = {}) {
  ...
  const game = {
    ...
    config: {
      ...
      /* one round a day, closing at this UTC hour */
...
/* One round a day, closing at the host's chosen hour. If that hour has already
   passed today the first round closes tomorrow — never in four minutes because
   the host happened to create the game at 17:56. */
export function nextDeadline(game, now) {
  ...
  while (d.getTime() - base.getTime() < MIN_LEAD_MS) {
    d.setTime(d.getTime() + game.config.cadenceHours * 3600 * 1000);
  }
