...
export const C = {
  ...
  /* ---- the warehouses ----------------------------------------------------
     ...
     RENT is $5, not the $10 the long board measured, and that is the whole
     balance of this feature: $10 a unit a round was calibrated against a
     SIXTEEN-round season and ours is eight, so a pile pays rent for a quarter
     of the season it was priced for. At $10 a keeper finishes $3,527 behind an
     anticipator on our table; at $5 they finish $592 behind, which is nothing.
     Half the season, half the accrual. (CARGO-findings-5.md, second pass.)

     ROYALTY is 40% — the top of the measured range, and measured again here.
     A 55% cut would also balance it and should not be the answer: at 55% a
     carrier who happens to buy from somebody's pile hands over more than half
     the sale without ever knowing whose it was, which is the fiction paying
     for a balance problem. */
  RENT: 5,           // per unit, per round, on everything still in a shed
  ROYALTY: 0.40,     // the keeper's cut of what their goods finally sell for
...
export const FORMAT = {
  ...
  rounds: 8,
  cadence: '5m',
  /* On, including ranked. Measured for this format rather than inherited from
     the long board: rent $5 and a 40% cut put a keeper level with an
     anticipator over eight rounds (CARGO-findings-5.md, second pass). */
  warehouses: true,
};
...
const humanSeat = (name, token) => ({
  ...
  stored: 0, royalties: 0, rentPaid: 0,
});
  ...
  if (game.config.warehouses) {
    ...
    if (seat.habit === 'keeper' && canStore(game)) {
      ...
      if (cheap > 1.02) {
        ...
        store = Math.min(C.STORE_MAX, Math.floor(spare * 0.35 / Math.max(bp, 1)));
      }
    }
  }
...
export function resolveStage(game, now) {
  ...
  if (wh) for (const x of plans) {
    ...
    x.s.stored = Math.max(0, (x.s.stored || 0) - (x.collect - left));
  }
  ...
  const rows = plans.map((x) => {
    ...
    /* WHOSE GOODS THOSE WERE. What a carrier buys at a rock comes out of the
       piles sitting there first, and the owner takes a cut of what it finally
       sells for — without the carrier ever learning whose it was. The keeper
       is paid for the wait and the risk; the carrier still keeps the margin on
       everything they bought from the market itself. */
    const from = wh ? drawFromPiles(game, dec.p, dec.c, bought) : [];
    const royalties = [];
    for (const src of from) {
      if (src.owner === game.seats.indexOf(s)) continue;   /* your own, no cut */
      const cut = C.ROYALTY * src.units * got;
      game.seats[src.owner].cash += cut;
      game.seats[src.owner].royalties = (game.seats[src.owner].royalties || 0) + cut;
      game.seats[src.owner].stored = Math.max(0, (game.seats[src.owner].stored || 0) - src.units);
      profit -= cut;
      royalties.push({ owner: src.owner, units: src.units, cut: Math.round(cut) });
    }

    s.cash += profit; s.at = rt.d;
    return { name: s.name, p: dec.p, c: dec.c, d: rt.d, load, paid, got, profit,
             bought, collect, fromPiles: from.reduce((a, f) => a + f.units, 0), royalties,
...
export function viewFor(game, token) {
  ...
  const view = {
    ...
    econ: { LOAD: C.LOAD, FUEL: C.FUEL, UPKEEP: C.UPKEEP,
            ...
            RENT: C.RENT, ROYALTY: C.ROYALTY, STORE_MAX: C.STORE_MAX },
    /* THE SHEDS.
    ...
    sheds: game.config.warehouses ? {
      ...
      windDown: C.WIND_DOWN, rent: C.RENT, royalty: C.ROYALTY, max: C.STORE_MAX,
      ...
      stored: me ? Math.round(me.stored || 0) : 0,
      royalties: me ? Math.round(me.royalties || 0) : 0,
