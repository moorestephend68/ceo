...
export const PRODUCTS = {
  host: {
    ...
    blurb: 'Your company name, kept for good, and the ability to host private games for your friends.',
  },
  facilitator: {
...
export async function startCheckout({ stripe, db, user, kind = 'host', companyName, origin, now }) {
  ...
  if (kind === 'host') {
    ...
    const existing = (await db.companiesOf(user.id)).find((c) => c.status === 'active');
    if (existing) throw new Error(`You already have a company: ${existing.name}.`);
    hold = await A.claimName(db, user.id, companyName, now);
  }
...
export async function applyEvent(db, event) {
  ...
  const kind = (s.metadata && s.metadata.kind) || 'host';
