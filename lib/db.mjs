...
export function memoryDb() {
  ...
  const invitations = [];               // one row per approach
  const results = [];
  ...
  return {
    ...
    async putGame(game, host) {
      ...
      /* THE STAND-IN HAS TO BE AS STRICT AS THE REAL THING.

         This one was not, and it cost a live game. Postgres types every column
         in db/schema.sql; a Map does not type anything. So a cargo table whose
         `stage` was the string 'declare' wrote happily here, passed every
         test, and was rejected by the database the instant it reached one — a
         failure that could only ever be found by playing the deployed game.

         Checking the types the schema declares turns that class of bug back
         into a test failure, which is where it belongs. */
      const typed = {
        code: 'string', status: 'string', is_public: 'boolean', stage: 'integer',
        deadline: 'string?', league: 'string?', cohort_id: 'string?',
        group_no: 'number?', kind: 'string?',
      };
      const check = (row) => {
        for (const [col, want] of Object.entries(typed)) {
          const v = row[col];
          const opt = want.endsWith('?');
          const base = opt ? want.slice(0, -1) : want;
          if (v === null || v === undefined) {
            if (opt) continue;
            throw new TypeError(`games.${col} is not nullable (got ${v})`);
          }
          const okType = base === 'integer'
            ? Number.isInteger(v)
            : typeof v === base;    // eslint-disable-line valid-typeof
          if (!okType) {
            throw new TypeError(
              `games.${col} is ${base} in db/schema.sql, but this write is `
              + `${JSON.stringify(v)} (${typeof v}) — Postgres would refuse it`);
          }
        }
      };
      ...
      const row = {
        ...
        stage: Number.isInteger(game.stage) ? game.stage : ((prev && prev.stage) || 0),
    ...
    async createEntrant(row) {
      const key = `${row.cohort_id}::${String(row.name).trim().toLowerCase()}`;
      for (const e of entrants.values()) {
        if (`${e.cohort_id}::${e.name.trim().toLowerCase()}` === key) {
          throw new UniqueViolation(row.name);
        }
      }
      const full = { id: id(), created_at: new Date().toISOString(), ...row };
      entrants.set(full.id, full);
      return { ...full };
    },

    async entrantsOf(cohortId) {
      return [...entrants.values()].filter((e) => e.cohort_id === cohortId)
        .sort((a, b) => String(a.created_at).localeCompare(String(b.created_at)))
        .map((e) => ({ ...e }));
    },

    async entrantByToken(token) {
      for (const e of entrants.values()) if (e.token === token) return { ...e };
      return null;
    },
    ...
    /* Create the game for one group exactly once, however many students arrive
       ...
       `stage` is 0 for a class, which has one game per group and always did.
       A tournament re-draws every stage, so the same group number comes round
       again and the identity has to include which stage it belongs to. */
    async ensureCohortGame(cohortId, groupNo, make, stage = 0) {
      for (const row of games.values()) {
    ...
    /* Every game opened inside a window, for /api/pulse. The whole state comes
       back because the seat list and the round number live in it; at the volume
       this question is asked about, that is a handful of rows. */
    async gamesSince(since, limit = 500) {
      const out = [];
      for (const row of games.values()) {
        if (Date.parse(row.created_at || 0) < Date.parse(since)) continue;
        out.push({ code: row.code, status: row.status, is_public: row.is_public,
                   cohort_id: row.cohort_id, league: row.league,
                   created_at: row.created_at, state: JSON.parse(row.state) });
        if (out.length >= limit) break;
      }
      return out;
    },

    async resultsSince(since, limit = 500) {
      return results
        .filter((r) => Date.parse(r.created_at || 0) >= Date.parse(since))
        .slice(0, limit)
        .map((r) => ({ company_id: r.company_id, league: r.league, created_at: r.created_at }));
    },

    async companyById(id) {
      for (const c of companies.values()) if (c.id === id) return { ...c };
      return null;
    },

    /* One approach per recruiter per player, enforced by the write rather than
       by checking first — two requests in the same instant cannot both win. */
    async putInvitation(row) {
      const clash = invitations.some((i) => i.recruiter === row.recruiter
                                         && i.company_id === row.company_id);
      if (clash) return null;
      const made = { id: 'inv_' + (invitations.length + 1), seen_at: null,
                     dismissed_at: null, ...row };
      invitations.push(made);
      return { ...made };
    },

    async invitationsSentSince(recruiter, since) {
      return invitations.filter((i) => i.recruiter === recruiter
        && Date.parse(i.created_at) >= Date.parse(since)).length;
    },

    /* Which players this recruiter has already approached, so the interface can
       grey them out instead of letting somebody spend an invitation to find
       out it was refused. */
    async invitationsSentBy(recruiter) {
      return invitations.filter((i) => i.recruiter === recruiter).map((i) => i.company_id);
    },

    async invitationsFor(companyId) {
      return invitations.filter((i) => i.company_id === companyId)
        .sort((a, b) => Date.parse(b.created_at) - Date.parse(a.created_at))
        .map((i) => ({ ...i }));
    },

    async dismissInvitation(id, companyId, at) {
      const row = invitations.find((i) => i.id === id && i.company_id === companyId);
      if (!row) return false;
      row.dismissed_at = at;
      return true;
    },
  ...
  return {
    ...
    async putGame(game, host) {
      ...
      const patch = {
        ...
        /* CEO's `stage` column is an integer — which round of a tournament a
           game belongs to. Cargo Run's `game.stage` is a different thing with
           the same name: which WINDOW of a round is open, 'declare' or
           'route'. Writing that string into an integer column made Postgres
           reject every write from the moment a cargo table started, and the
           page, which swallows failed polls, simply froze mid-countdown.

           The cargo stage lives in `state` where it belongs. This column keeps
    ...
    async createEntrant(row) {
      const r = await client.from('entrants').insert(row).select().single();
      if (r.error) throw translate(r.error);
      return r.data;
    },

    async entrantsOf(cohortId) {
      return rows(await client.from('entrants').select('*')
        .eq('cohort_id', cohortId).order('created_at', { ascending: true })) || [];
    },

    async entrantByToken(token) {
      return one(await client.from('entrants').select('*')
    ...
    /* Unique on (cohort_id, stage, group_no), so two students opening group 3 at
       the same moment produce one group 3. The loser reads the winner's row.

       `stage` is 0 for a class and always was. A tournament re-draws every stage,
       so group 1 comes round again and the identity has to say which stage. */
    async ensureCohortGame(cohortId, groupNo, make, stage = 0) {
      const find = async () => one(await client.from('games').select('state, version')
        .eq('cohort_id', cohortId).eq('group_no', groupNo).eq('stage', stage).maybeSingle());
    ...
    async gamesSince(since, limit = 500) {
      return rows(await client.from('games')
        .select('code, status, is_public, cohort_id, league, created_at, state')
        .gte('created_at', since)
        .order('created_at', { ascending: false }).limit(limit)) || [];
    },
    ...
    async resultsSince(since, limit = 500) {
      return rows(await client.from('results')
        .select('company_id, league, created_at')
        .gte('created_at', since).limit(limit)) || [];
    },

    async companyById(id) {
      return one(await client.from('companies').select('*').eq('id', id).maybeSingle());
    },

    async putInvitation(row) {
      const r = await client.from('invitations').insert(row).select('id').single();
      /* The unique index on (recruiter, company_id) is the rule. A clash is the
         rule working, not a failure. */
      if (r.error) {
        if (r.error.code === '23505') return null;
        throw translate(r.error);
      }
      return r.data;
    },

    async invitationsSentSince(recruiter, since) {
      const r = await client.from('invitations')
        .select('id', { count: 'exact', head: true })
        .eq('recruiter', recruiter).gte('created_at', since);
      if (r.error) throw translate(r.error);
      return r.count || 0;
    },

    async invitationsSentBy(recruiter) {
      const data = rows(await client.from('invitations').select('company_id')
        .eq('recruiter', recruiter).limit(2000));
      return (data || []).map((r) => r.company_id);
    },

    async invitationsFor(companyId) {
      return rows(await client.from('invitations').select('*')
        .eq('company_id', companyId)
        .order('created_at', { ascending: false }).limit(50)) || [];
    },

    async dismissInvitation(id, companyId, at) {
      const r = await client.from('invitations').update({ dismissed_at: at })
